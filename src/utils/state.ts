...
import { deepEqual, listToDict, randomId } from "@/utils/helper"
...
function extractViewFromRelevantRawMap(selector: StateViewSelector, raw: AnyDict, tabId: number) {
	...
	for (let key in selector) {
		if (key === "isPinned") {
			...
			view[key] = ORL_CONTEXT_KEYS.some((o) => raw[exclaimIncipit.concat(o)] != null)
		} else if (CONTEXT_KEYS_SET.has(key as keyof Context)) {
...
export class SubscribeView {
	...
	constructor(
		_selector: StateViewSelector | string[],
		private tabId?: number,
		private onLaunch?: boolean,
		cb?: SubViewCallback,
		public wait?: number,
	...
	pushPreCog = async (override: StateView, knownPinStatus: boolean) => {
		const base = await pushViewGhost({
			override,
			tabId: this.tabId,
			knownPinStatus,
			overrideRaw: {
		...
		return Promise.all([this.handleChange(changes, true), chrome.storage.local.set(base)])
	}
}
...
export class SubscribeStorageKeys {
	...
	release = () => {
		...
		chrome.storage.local.onChanged.removeListener(this.handleChange)
		this.cbs.clear()
		;(delete this.cbs, delete this.rawMap, delete this.latestRaw, delete this.keys, delete this.rawMap)
	}
