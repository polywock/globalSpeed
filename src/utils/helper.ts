...
export function formatDuration(secs: number, includePositive?: boolean) {
	...
	if (hours >= 100) {
		...
		return `${prefix}${Math.floor(hours)}:${Math.floor(mins % 60)
			.toString()
			.padStart(2, "0")}:${Math.floor(secs % 60)
			.toString()
			.padStart(2, "0")}`
	} else {
		return `${prefix}${Math.floor(mins).toString().padStart(1, "0")}:${Math.floor(secs % 60)
			.toString()
			.padStart(2, "0")}`
	}
}
...
export async function createWindowWithSafeBounds(createData: chrome.windows.CreateData) {
	const hasBounds =
		Number.isFinite(createData.left) || Number.isFinite(createData.top) || Number.isFinite(createData.width) || Number.isFinite(createData.height)
