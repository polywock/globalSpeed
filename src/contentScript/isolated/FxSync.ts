...
export class FxSync {
	...
	handleChange = (view: StateView) => {
		let styleInfo = calculateStyle(
			view.elementFx.enabled,
			view.elementFx.query || "video",
			formatFilters(view.elementFx.filters),
			formatFilters(view.elementFx.transforms.slice().reverse()),
			`${view.elementFx.originX || "center"} ${view.elementFx.originY || "center"}`,
			view.elementFx.svgFilters,
		)
		...
		const backdropStyleInfo = calculateStyle(
			view.backdropFx.enabled,
			"placeholder",
			backdropSimpleFilter,
			backdropTransform,
			backdropTransformOrigin,
			view.backdropFx.svgFilters,
		)
