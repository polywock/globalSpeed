...
export class Interactive extends Popover {
	...
	isAtCancel = (xy: ReturnType<typeof extractClient>) => {
		...
		if (!b.width) return
		if (between(b.x, b.x + b.width, xy.clientX) && between(b.y, b.y + b.height, xy.clientY)) return true
	}
	...
	draw = (xy: ReturnType<typeof extractClient>, final?: boolean) => {
		...
		if (SLIDER_SHOW)
			this.slider.style.transform = `translateX(calc((100vw - ${2 * BLEEDING}px) * ${normal} + ${BLEEDING}px - ${SLIDER_WIDTH * 0.5}px))`
		return
	}
	cancel = () => {
