...
export class SmartFs {
	...
	attemptIntegration = async () => {
		...
		let matches = [...gvar.os.mediaTower.media]
			.filter((m) => m.readyState && m instanceof HTMLVideoElement && (m as HTMLVideoElement).videoWidth && m.isConnected)
			.map((m) => findFullscreenAncestor(m))
			.filter((v) => v)
...
export async function findPrimeProfile(child: Node, primeThreshold = 0.5): Promise<{ profile: FsProfile; ancestor: Element }> {
	...
	scores
		.filter((score) => score.score / score.profile.maxScore > primeThreshold)
		.forEach((score) => {
