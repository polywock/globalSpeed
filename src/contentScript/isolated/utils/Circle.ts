...
export class Circle extends Popover {
	...
	isAtCircle = (xy: ReturnType<typeof extractClient>) => {
		...
		if (between(x - this.size * half, x + this.size * half, clientX) && between(y - this.size * half, y + this.size * half, clientY)) return true
	}
	isAtDelete = (large?: boolean) => {
		let circleBounds = this.ref.getBoundingClientRect()
		let circleXY: ReturnType<typeof extractClient> = {
			clientX: circleBounds.x + circleBounds.width * 0.5,
			clientY: circleBounds.y + circleBounds.height * 0.5,
		}
	...
	getDirection = (clientX: number, clientY: number): Direction => {
		...
		const direction =
			Math.abs(deltaX) > Math.abs(deltaY) ? (deltaX >= 0 ? Direction.RIGHT : Direction.LEFT) : deltaY >= 0 ? Direction.BOTTOM : Direction.TOP
	...
	clearDirectional = () => {
		if (this.direction) {
			...
			clearInterval(this.directionalIntervalId)
			delete this.directionalIntervalId
		}
	}
