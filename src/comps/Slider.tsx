...
export function Slider(props: SliderProps) {
  ...
  env.handleChange = useCallback(debounce((value: number) => {
    const { props } = env 
    props.onChange(clamp(props.min, props.max, value))
  }, props.wait ?? 25, {maxWait: props.maxWait ?? 50, leading: true, trailing: true}), [])
  ...
  return (
    <input
      ...
      onBlur={clearAnchor} 
      type="range" min={min} max={max} step={step} value={props.value} onChange={e => {
