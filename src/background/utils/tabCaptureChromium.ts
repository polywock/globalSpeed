...
export const SUPPORTS_TAB_CAPTURE = !!(chrome.tabCapture?.capture && chrome.offscreen?.createDocument)
...
export async function initTabCapture(tabId: number): Promise<boolean> {
	...
	try {
		const [streamId, view] = await Promise.all([chrome.tabCapture.getMediaStreamId({ targetTabId: tabId }), fetchView(AUDIO_CONTEXT_KEYS, tabId)])
