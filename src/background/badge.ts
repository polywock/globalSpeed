...
async function getBadgeInit(tabId: number) {
	const { isPinned, speed, enabled, hasOrl, superDisable, hideBadge } = await fetchView(
		{ hideBadge: true, superDisable: true, isPinned: true, speed: true, enabled: true, hasOrl: true },
		tabId,
	)
