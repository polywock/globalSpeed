...
import { AnyDict, CONTEXT_KEYS, Context, InitialContext, Keybind, KeybindMatch, KeybindMatchId, State, StateView, Trigger, URLRule} from "@/types"
import { PREFIX_SETS, dumpConfig, fetchView, getKeysByPrefix, pushView, restoreConfig } from "@/utils/state"
...
import { MediaEvent } from "@/contentScript/isolated/utils/applyMediaEvent"
    ...
    if (msg.type === "REQUEST_TAB_INFO") {
       ...
       gvar.es.set(msg.override)
        ...
        chrome.webNavigation?.getAllFrames({tabId: sender.tab.id}, frames => {
            frames.map(f => f.frameId).filter(id => id).forEach(frameId => {
                chrome.tabs.sendMessage(sender.tab.id, {
                    type: "TOP_FRAME_URL_UPDATE",
                    value: msg.value 
                } as Messages, {frameId})
